...
// ── FULL SUBSCRIPTION — reads groups + trasferte separately ───
// Avoids reading piano/v2 root (not covered by Firebase rules)
function dbSubscribe(onData, onError) {
  var latestGroups    = {};
  var latestTrasferte = {};
  var groupsReady     = false;
  var trasferteReady  = false;

  function emitIfReady() {
    if (groupsReady && trasferteReady) {
      onData(normalizeDbData({ groups: latestGroups, trasferte: latestTrasferte }));
    }
  }

  // Subscribe to groups
  dbRef("groups").on("value",
    function(snap) {
